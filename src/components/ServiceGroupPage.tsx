...
async function getServices(group: ServiceGroup): Promise<Service[]> {
  try {
    ...
    return data
      .filter((service) => service.published && service.group === group)
...
export default async function ServiceGroupPage({
  ...
  const services = await getServices(group);
          ...
          {services.length === 0 ? (
            <div className="rounded-2xl border bg-white px-5 py-16 text-center shadow-sm">
              ...
              <p className="mt-2 text-sm text-gray-500">
                Service information will appear here once it is published.
              </p>
            </div>
          ) : (
            <div className="grid gap-7 lg:grid-cols-[280px_minmax(0,1fr)]">
              ...
              <aside>
                <div className="overflow-hidden rounded-2xl border bg-white shadow-sm lg:sticky lg:top-24">
                  <div className="bg-[#1F3C88] px-5 py-4 text-lg font-bold text-white">
                    {title}
                  </div>
                      ...
                      return (
                        <Link
                          ...
                          className={`block break-words px-5 py-4 text-sm transition ${
                            active
                              ? "bg-green-50 font-semibold text-[#1F3C88]"
                              : "text-gray-700 hover:bg-gray-50 hover:text-[#1F3C88]"
                          }`}
                        >
                          {service.title}
                        </Link>
              ...
              {selectedService && (
                <div className="min-w-0">
                  ...
                  {selectedService.type === "external-link" && (
                    <article className="rounded-2xl border bg-white px-6 py-12 text-center shadow-sm">
                      {selectedService.externalUrl ? (
                        <a
                          ...
                          className="inline-flex rounded-xl bg-[#1F3C88] px-7 py-3 font-semibold text-white transition hover:bg-[#162E6A]"
                        >
                          {selectedService.buttonText || "Open Link"}
                        </a>
                      ) : (
